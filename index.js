...
var chain_folder = process.env.BITCOIN_CHAIN_FOLDER || "~/.qtum"
var bitcoin_rpc_user = process.env.BITCOIN_RPC_USER || "qtumtest"
var bitcoin_rpc_password = process.env.BITCOIN_RPC_PASSWD || "qtumtest"

var ord_binary = process.env.ORD_BINARY || "./qord"
var ord_folder = process.env.ORD_FOLDER || ""
var ord_datadir = process.env.ORD_DATADIR || "."
var rpc_url = process.env.RPC_URL || "http://127.0.0.1:13889"
const network_type = process.env.NETWORK_TYPE || "testnet"

var network = null
var network_folder = ""
if (network_type == "mainnet") {
    network = qtumjs.networks.qtum
    network_folder = ""
...
const ORD_VERSION = 'ord 0.15.0-qtum'
...
async function main_index() {
    ...
    while (true) {
        ...
        let ord_last_block_height = ord_last_block_height_q[0].max_height
       ...
       if (network == qtumjs.networks.qtum_testnet) {
        ...
        let ord_index_cmd = ord_binary + network_argument + " --bitcoin-data-dir " +
            chain_folder + " --data-dir " + ord_datadir + " --rpc-url " + rpc_url + " --height-limit " +
            (ord_end_block_height) + " " + rpc_argument + " index run"
        try {
        ...
        for (const l of lines_index) {
            ...
            if (parts[2].trim() == "new_block") {
                ...
                let blockhash_db_q = await new Promise((resolve, reject) => {
                    db_pool.query("select block_hash from block_hashes where block_height = ?;", [block_height], (error, results, fields) => {
                        if (error) {
                            console.error(error.message);
                            reject(error);
                        } else {
                ...
                if (blockhash_db_q[0].block_hash != blockhash) {
                    ...
                    await new Promise((resolve, reject) => {
                        db_pool.query(`INSERT into ord_indexer_reorg_stats
                                           (reorg_tm, old_block_height, new_block_height)
                                       values (?, ?, ?);`,
                            [reorg_tm, current_height, block_height - 1], (error, results, fields) => {
                                if (error) {
                                    console.error(error.message);
                                    reject(error);
                                } else {
        ...
        if (!next_expected_start) {
            ...
            await new Promise((resolve, reject) => {
                db_pool.query(`INSERT into ord_indexer_work_stats
                                   (ord_index_tm, all_tm)
                               values (?, ?);`,
                    [ord_index_tm, all_tm], (error, results, fields) => {
                        if (error) {
                            console.error(error.message);
                            reject(error);
                        } else {
        ...
        let current_transfer_id = parseInt(current_transfer_id_q[0].maxid) + 1
        ...
        await new Promise((resolve, reject) => {
            db_pool.query(`INSERT into ord_indexer_work_stats
                           ...
                           values (?, ?, ?, ?, ?, ?, ?, ?, ?);`,
                [main_min_block_height, main_max_block_height, ord_sql_query_count, new_inscription_count,
                    transfer_count, ord_index_tm, ord_sql_tm, update_log_tm, all_tm], (error, results, fields) => {
                    if (error) {
                        console.error(error.message);
                        reject(error);
                    } else {
...
async function check_db() {
    ...
    try {
        let sqlString = 'SELECT db_version from ord_indexer_version;'
        let db_version = await new Promise((resolve, reject) => {
            db_pool.query(sqlString, (error, results, fields) => {
                if (error) {
                    console.error(error.message);
                    reject(error);
                } else {
                    console.log(results[0].db_version);
                    resolve(results[0].db_version);
                }
            });
        });
    ...
    let network_type_db = res_q[0].network_type
    ...
    let current_transfer_height = current_transfer_height_q[0].max_height
    ...
    let current_ord_number_to_id_height = current_ord_number_to_id_height_q[0].max_height
    ...
    let current_content_height = current_content_height_q[0].max_height
    ...
    await new Promise((resolve, reject) => {
        ...
        resolve()
        //
        // db_pool.query(`INSERT into ord_indexer_version
        //   (id, indexer_version, db_version)
        //   values (3, ?, ?);`, ["123", 3],(error, results, fields) => {
        //     if (error) {
        //         console.error(error.message);
        //         reject(error);
        //     } else {
